...
SELECT
    ...
    CONCAT(employees.first_name, ' ', employees.last_name) AS fillname
FROM
    orders
    JOIN employees USING(employee_id)
    JOIN customers USING(customer_id)
    JOIN shippers ON shippers.snipper_id = orders.ship_via
WHERE
...
SELECT
    ...
    supliers.contact_name AS sup_name,
    supliers.phone AS sup_phone
FROM
    products
    JOIN supliers USING (suplier_id)
    JOIN caategories USING (category_id)
WHERE
